...
# Constants
MODEL = "deepseek-reasoner"  # Use the correct model name for DeepSeek
...
DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY")  # Load API key from .env file

if not DEEPSEEK_API_KEY:
    print(colored("Error: DEEPSEEK_API_KEY is not set in the .env file.", "red"))
    exit(1)
...
def generate_code(prompt, iteration=1):
    ...
    try:
        ...
        # Retrieve both Chain of Thought (CoT) and the final answer
        reasoning_content = response.choices[0].message.reasoning_content
        content = response.choices[0].message.content
        print(colored(f"Reasoning Content (CoT): {reasoning_content}", "blue"))  # Debug output
        return content
    except Exception as e:
